...
# --- Thresholds (you can adjust these as needed) ---
thresh_gp = 40000
...
if uploaded_file is not None:
    try:
        ...
        if marcus_df.empty:
            ...
            # Standardize and convert necessary fields
            ...
            marcus_df['SMT Qty'] = pd.to_numeric(marcus_df['SMT Qty'], errors='coerce')
            marcus_df['GP Per SMT'] = np.where(marcus_df['SMT Qty'] > 0, marcus_df['GP'] / marcus_df['SMT Qty'], 0)

            # Most recent row (assuming one row per month)
            ...
            summary_data = {
                ...
                "Met?": [
                    "Yes" if marcus['GP'] >= thresh_gp else "No",
                    "Yes" if marcus['VZ Perks Rate'] >= thresh_vmp else "No",
                    "Yes" if marcus['GP Per SMT'] >= thresh_gp_per_smt else "No",
                    vhi_fios_met
                ]
            }
